...
var renderScreen = (PORT, mappings) => {
  console.clear();
...
`);
  if (mappings.size === 0) {
    ...
    console.table(Array.from(mappings.entries()).map(([path2, conf]) => {
      if (conf.target) {
        return { Path: path2, "→ Proxy To": conf.target };
      } else {
        return {
          Path: path2,
...
var createOrUpdateProxies = (inputPath, target, pathRewrite) => {
...
rl.question(`Enter hosting port (default ${hostPort}): `, (answer) => {
  ...
  app.use((req, res, next) => {
    const matchedEntry = [...mappings.entries()].sort((a, b) => b[0].length - a[0].length).find(([path2]) => req.path.startsWith(path2));
    if (!matchedEntry) {
